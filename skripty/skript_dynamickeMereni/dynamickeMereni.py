...
def graf_Q(podlazi, Dates, Q):
    ...
    for i in np.arange(len(Q_n)):
        ...
        # ax.plot(Dates[i],Q_n[i], label=str(podlazi[i]))
    ax.set_xlabel("$datum$")
    ...
    format_data = mdates.DateFormatter('%d. %m. %Y')
...
def export_R(R,V,N):
    ...
    export prutoku
    '''
    def vymena_vzduchu(R=R, V=V, N=N):
        n=0
        for i in np.arange(1, N+1):
    ...
    R=pd.DataFrame(np.array([unumpy.nominal_values(R), unumpy.std_devs(R)]).T[0], columns=['hodnota $\left[\si{m^3/hod}\right]$', r'$\sigma$'],
                   index=R.index)
    R.to_latex('vysledky_prutoky.tex', decimal=',',float_format='%0.3f', escape=False)
    return 0
...
a_out=0
N, podlazi, Dates, A, A_diff, V, R, Q=run(umisteni_sond, a_out)
