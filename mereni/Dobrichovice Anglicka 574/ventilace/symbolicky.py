...
#vypocet doby mereni
print('Nezapomen upravit dobu mereni!!!')
pocatek=datetime.datetime(2019, 7, 9, 13, 00)
konec=datetime.datetime(2019, 7, 30, 14, 00)
...
odpary_err =odezva_TD.loc[:,'odpar_err'] #N_plyny x N

R_d=completion(R_d, R_d_err)
odpary=completion(odpary, odpary_err)
...
prutoky_hodnoty.to_csv('airflows.txt', sep=';', decimal=',', float_format='%.8f')

pouzite_plyny.to_csv('airflows_ID.csv', header=False, sep=';')
toc=time.time()
print('Uplynuly cas: '+str(toc-tic))
odezvy_TD=pd.DataFrame(unumpy.uarray(odezva_TD.loc[:, 'R_d'], odezva_TD.loc[:, 'R_d_err']),
                       index=[odezva_TD.loc[:,'jmeno'], list(np.arange(1, N+1))*N], columns=[r'$R$ [ng]'])

odezvy_TD.index.set_names(['plyn', 'zóna'], inplace=True)

def f(x):
    return '{:.1f}'.format(x)

odezvy_TD.to_latex('odezvy_TD.tex', escape=False, formatters=[f])
