...
Z_n.columns+=1
Z_s=Z_n.copy()
Z=pd.DataFrame(np.zeros((N*N_plyny, pocet_neznamych)), index=odezva_TD.index,
...
# prutoky_hodnoty2=pd.DataFrame(np.array([prutoky3_n, prutoky3_s]).T, index=prutoky_indexy_modified,
                            # columns=['R', 'uR'])
prutoky_hodnoty2=pd.DataFrame(np.array([unumpy.nominal_values(prutoky2),
                                        unumpy.std_devs(prutoky2)]).T,
                              index=prutoky_indexy_modified,
                            columns=['R', 'uR'])
