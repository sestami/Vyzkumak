...
Q_means_list.append([Q[:,0].mean(), Q[:,1].mean(), Q[:,2].mean(), Q[:,3].mean()])
...
for el in airflows_ID[1:]:
    ...
    Q_means_list.append([Q0.mean(), Q1.mean(), Q2.mean(), Q3.mean()])
    airflows_combination_list.append(airflows_combination)
...
def sloupce(patro):
        return r'$Q_'+str(patro)+r'$ $\left[\si{\frac{Bq}{m^3\cdot hod}}\right]$'
columns=[]
for patro in np.arange(1, len(podlazi)+1):
    columns.append(sloupce(patro))

Q_df=pd.DataFrame(Q_means_list, index=airflows_combination_list, columns=columns)
                  # columns=[r'$Q_0$ $\left[\si{\frac{Bq}{hod}}\right]$',
                                                                          # r'$Q_1$ $\left[\si{\frac{Bq}{hod}}\right]$',
                                                                          # r'$Q_2$ $\left[\si{\frac{Bq}{hod}}\right]$'])
Q_df.to_latex('vysledky_Q_dynamicky.tex', formatters=N*[g], escape=False)
