...
veliciny=np.array([a,V,a_diff]) #mineno vstupni veliciny, Q je hledana velicina
...
for i, el_i in enumerate(veliciny.flatten()):
    for j, el_j in enumerate(veliciny.flatten()):
        if j>=i:
...
'''
mam o jednu rovnici navic -> ta je pro venkovni prostredi, neni potreba...
ale zatim ji tam nechavam
'''
eqn=np.array([])
for i, Q_i in enumerate(Q):
    rce=(V[i]*(a_diff[i]+prem_kons_Rn*a[i])+a[i]*sum(K[i,:])-sum(a[:]*K[:,i])-Q_i)
...
for i in np.arange(len(dQdx_matrix)):
    for j, velicina in enumerate(veliciny.flatten()):
...
# for i, dQ_i in enumerate(dQdx_matrix):
    # for j, dQ_j in enumerate(dQdx_matrix):
        # cov_matrix_Q[i, j]=sum(np.dot(diff(Q_i,veliciny.flatten()),))
