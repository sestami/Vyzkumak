...
Výpočetní model přísunů radonu jsem implementoval rozděleně pro případ měření v rovnovážném stavu a pro případ dynamického měření v programovacím jazyce Python. Skript pro vyhodnocení dynamického měření je v příloze~\ref{navesti:priloha_dynamickeMereni}, skript pro vyhodnocení měření v rovnovážném stavu je pouze jeho jednodušší varianta, a proto ho v této práci neuvádím.
...
\begin{itemize}
    ...
    \item\emph{airflows.txt} obsahuje průměrné průtoky mezi všemi zónami (tedy i vnějším prostředím),
    \item\emph{concentrations.txt} obsahuje průměrné OAR ve všech zónách objektu, tedy OAR vnějšího prostředí není zahrnuto.
\end{itemize}

Poté skript vytvoří matici $\mathbb{K}$ a vektor $\vv{a}$, z nichž pomocí rovnice \eqref{eq:maticovy_zapis_rovnovaha} určí $\vv{Q}$. Nejistoty jsou propagovány pomocí Python balíčku uncertainties~\cite{uncertainties}.
