...
\begin{labeling}{RaA+RaC}
    \item[RaA] rychlejší, ale méně přesný způsob. OAR se počítá pouze z naměřených impulzů příslušejících \ce{^{218}Po}.
    \item[RaA+RaC] pomalejší, ale přesnější způsob. OAR se počítá z impulzů příslušejících \ce{^{218}Po} i \ce{^{214}Po}.
\end{labeling}
...
Jak již bylo řečeno, TERA sondy zapisují průměrnou hodnotu OAR každou hodinu. Spolu s OAR jsou zapisovány i naměřené počty impulzů v jednotlivých diskriminačních oknech, která mají TERA sondy nastaveny pro rozlišení impulzů: do druhého diskriminačního okna spadají impulzy od \ce{^{218}Po} (\SI{6}{MeV}), do třetího od \ce{^{214}Po} (\SI{7.69}{MeV}), do prvního a čtvrtého impulzy od ostatních energií, jenž nás nezajímají. Díky tomu je možné vypočítat nejistotu naměřeného počtu impulzů náležejících \ce{^{218}Po} a \ce{^{214}Po}, označme jej $n$. Počet zaznamenaných impulsů se řídí Poissonovým rozdělením, a proto pro směrodatnou odchylku $n$ máme:
\begin{equation}
    \sigma_n=\sqrt{n}\,.
...
Jedná se o přesné kalibrované zdroje s deklarovanou aktivitou \ce{^{226}Ra} a radonovou výdejností. Radonová výdejnost je množství radonu, které se ze zdroje za pomoci čerpadla dostane ven za jednotku času, a udává se v jednotkách v \si{Bq/s}. Zdroj je k nahlédnutí v obr.~\ref{fig:radon_zdroj}.
